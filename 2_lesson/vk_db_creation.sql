...
-- таблица с учебными заведениями пользователя. связь м-м
DROP TABLE IF EXISTS education;
CREATE TABLE education (
	id SERIAL PRIMARY KEY,
	name VARCHAR(150) COMMENT 'название учебного заведения',
	INDEX education_name_idx(name)
);
...
CREATE TABLE user_education (
	...
	FOREIGN KEY (user_id) REFERENCES users(id) ON UPDATE CASCADE ON DELETE CASCADE,
	FOREIGN KEY (education_id) REFERENCES education(id) ON UPDATE CASCADE ON DELETE CASCADE
);

-- таблица с добавлением музыки, связь м-м
DROP TABLE IF EXISTS music;
CREATE TABLE music (
	id SERIAL PRIMARY KEY,
    music_id BIGINT UNSIGNED,
    user_id BIGINT UNSIGNED NOT NULL,
    downloaded_at DATETIME DEFAULT NOW() COMMENT 'когда добавлено',
...
-- таблица с локацией, связь м-м
DROP TABLE IF EXISTS location;
CREATE TABLE location (
	id SERIAL PRIMARY KEY,
	country_name VARCHAR(150),
	town_name VARCHAR(150),
	INDEX location_idx(country_name, town_name)
);
